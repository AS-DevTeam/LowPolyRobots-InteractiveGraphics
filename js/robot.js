...
function createEye(robotSizes){
  ...
  // Create outline object
  const outlineW = wEyeSup * 0.05;
  const outlineH = hEyeSup * 0.05;
  const outlineD = dEyeSup * 0.05;

  const outline_geo = new THREE.BoxGeometry(wEyeSup + outlineW, hEyeSup + outlineH, dEyeSup + outlineD);
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  const outline = new THREE.Mesh( outline_geo, outline_mat );
  eyeSuppMesh.add(outline);
...
function createNeck(robotSizes){
  ...
  // Create outline object
  const outlineRadius = radius * 0.05;
  const outlineTube = tube * 0.05;
  const outlineRadSeg = radialSeg * 0.05;
  const outlineTubSeg = tubularSeg * 0.05;

  const outline_geo = new THREE.TorusGeometry(
    radius + outlineRadius,
    tube + outlineTube,
    radialSeg + outlineRadSeg,
    tubularSeg + outlineTubSeg
  );
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  const outline = new THREE.Mesh( outline_geo, outline_mat );

  const upperNeck = new THREE.Mesh(neckGeo, neckMat);
  upperNeck.add(outline);

  const middleNeck = new THREE.Mesh(neckGeo, neckMat);
  middleNeck.add(outline);

  const lowerNeck = new THREE.Mesh(neckGeo, neckMat);
  lowerNeck.add(outline);
...
function createShoulder(robotSizes){
  ...
  let outline_geo, outline, outlineH;
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  ...
  // Create outline object
  const outlineW = w * 0.05;
  outlineH = h * 0.05;
  const outlineD = d * 0.05;

  outline_geo = new THREE.BoxGeometry(w + outlineW, h + outlineH, d + outlineD);
  ...
  // Create outline object
  const outlineRtop = r * 0.05;
  const outlineRbot = r * 0.05;
  outlineH = height * 0.05;
...
function createArm(robotSizes){
  ...
  // Create outline object
  const outlineW = w * 0.5;
  const outlineH = h * 0.5;
  const outlineD = w * 0.5;

  const outline_geo = new THREE.BoxGeometry(w + outlineW, h + outlineH, w + outlineD);
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  const outline = new THREE.Mesh( outline_geo, outline_mat );
  const outlineBack = new THREE.Mesh( outline_geo, outline_mat );
  mesh.add(outline);
  meshBack.add(outlineBack);
...
function createCannon(robotSizes){
  ...
  let outlineRtop, outlineRbot, outlineH, outline_geo, outline;
  ...
  // Create outline object
  outlineRtop = rTop * 0.05;
  outlineRbot = rBot * 0.05;
  outlineH = h * 0.05;
...
function createTorso(robotSizes){
  ...
  const outlineH = height * 0.05;
  const outlineD = width * 0.05;

  const outline_geo = new THREE.BoxGeometry(width + outlineW, height + outlineH, width + 0.5 + outlineD);
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  const outline = new THREE.Mesh( outline_geo, outline_mat );
  mesh.add(outline);
...
function createUpperLeg(robotSizes){
  ...
  const outline_geo = new THREE.BoxGeometry(width + outlineW, height + outlineH, width + 0.1 + outlineD);
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  const outline = new THREE.Mesh( outline_geo, outline_mat );
  mesh.add(outline);
...
function createMidLeg(robotSizes){
  ...
  const outline_geo = new THREE.BoxGeometry(width + outlineW, height + outlineH, width + outlineD);
  const outline_mat = new THREE.MeshBasicMaterial({ color : 0x0000000, side: THREE.BackSide });
  const outline = new THREE.Mesh( outline_geo, outline_mat );
  mesh.add(outline);
...
function createLowerLeg(robotSizes){
  ...
  // Create outline object
  const outlineRtop = radius * 0.05;
  const outlineRbot = radius * 0.05;
  const outlineH = heightCyl * 0.05;
